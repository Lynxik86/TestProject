...
@Singleton
class ChuckTasksRepository @Inject internal constructor(//val connectDb: ConnectDb,
                                                        private val chuckLocalDataSource: ChuckLocalDataSource,
                                                        private val chuckRemoteDataSource: ChuckRemoteDataSource
) : ChuckDataSource {

    /*companion object {
        @Volatile
        private var INSTANCE: ChuckTasksRepository? = null

        fun getInstance(connectDb: ConnectDb): ChuckTasksRepository =
            INSTANCE ?: synchronized(this) {
                INSTANCE ?: ChuckTasksRepository(connectDb).also { INSTANCE = it }
            }
    }*/
...
/*    private val chuckLocalDataSource: ChuckLocalDataSource =
    ...
    override suspend fun getChuck(): ChuckResult {
        val chuckResult = chuckRemoteDataSource.getChuckResult()
        return chuckLocalDataSource.postChuck(chuckResult)
    }
